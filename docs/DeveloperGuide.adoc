...
* As long as the customer and list of dish fields are not empty, the command `draftpoints` can be used to add a Points object from the Member class into the draft order. The Points object is referenced from the Points object in the Member class that was added by using the `draftcustomer` command.
